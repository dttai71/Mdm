...
> **Per @mis + CEO rider #3 (29/06) — Recipe_BTP cột L+M+N empirical gap closure (handoff `7abd463e`)**: Sheet `Định lượng BTP` source (`core/docs/10-Temporary/Master Data NVL.xlsx`) verified — col L=`Tổng Định lượng thành phẩm` (yield_qty) + col M=`DVT thành phẩm` (yield_uom) + col N=`Đơn giá sau chế biến` (BTP unit-price-fact, = K÷L) PRESENT in header-row-per-BTP-group pattern (e.g. INT-0006 Bò một nắng muối kiến vàng: yield=1200gr, DVT=gr, unit=478.55đ/gr in row L11; component rows L12+ NULL). MASTER-DATA-V2.xlsx Recipe_BTP previously MISSING all 3 columns — Kimi 29/06 import (per @mis handoff QA-KIMI-HANDOFF-RECIPE-BTP-KEEP-YIELD-28062026.md) adds: `Tong_Dinh_Luong_Thanh_Pham` + `DVT_Thanh_Pham` + `Don_Gia_Sau_Che_Bien` (FLATTEN value, 0-formula, denormalize per Ma_BTP, drop derived J/K). SPEC-002 mdg_005_btp + SPEC-005 §3.2 + SPEC-006 §6.2 specs ALL canonical pre-update — design assumed columns exist. **Authoritative chain interim Phase 1**: BTP cost = `mdg_005_btp.don_gia_sau_che_bien` Excel-authoritative direct lookup (parallel role NVL cột-K); engine yield-divide compute = VALIDATION pattern (`engine_recompute ≈ Excel cột-N ±tolerance`). Phase 2 OR cột-N stale → engine compute via full yield-divide (SPEC-006 §6.2 unchanged). **Parser convention**: yield + cột-N in HEADER ROW per BTP group only; parser stores `mdg_005_btp.yield_qty/uom/don_gia_sau_che_bien` once per BTP. NULL on any BTP → anomaly `RECIPE_INVALID_REF (yield missing)` per SPEC-006 §6.2.
